...
def normalize_symptom_name(name: str) -> str:
    """Normalize symptom names: remove all spaces, lowercase.

    Fixes mismatches like 'dischromic _patches' vs 'dischromic_patches',
    'spotting_ urination' vs 'spotting_urination',
    'foul_smell_of urine' vs 'foul_smell_ofurine'.
    """
...
def load_severity_weights(path: Path) -> dict[str, int]:
    ...
    # Fix 1: Remove spurious 'prognosis' entry (metadata artifact, not a symptom)
    df = df[df["Symptom"] != "prognosis"]

    # Fix 2: Normalize symptom names to handle internal space mismatches
    #   e.g. "dischromic _patches" → "dischromic_patches"
    df["Symptom"] = df["Symptom"].apply(normalize_symptom_name)

    # Fix 3: Deduplicate (e.g. fluid_overload appears twice with weights 6 and 4)
    #   Keep the max weight for safety
    df = df.groupby("Symptom", as_index=False)["weight"].max()
...
def compute_urgency(disease: str, row: pd.Series, severity_weights: dict) -> str:
    ...
    for col in symptom_cols:
        ...
        if symptom and symptom != "nan":
            # Fix 4: Normalize symptom name to match normalized severity keys
            normalized = normalize_symptom_name(symptom)
            total_severity += severity_weights.get(normalized, 0)
...
def oversample_by_key(records: list[dict], key: str) -> list[dict]:
    ...
    for label, items in groups.items():
        if len(items) < max_count:
            # Repeat items to reach max_count
            oversampled = items * (max_count // len(items))
            oversampled += random.sample(items, max_count - len(oversampled))
...
def main():
    ...
    # Train / test split (split BEFORE oversampling to avoid data leakage)
    random.seed(42)
    random.shuffle(records)
    split = int(len(records) * 0.85)
    train, test = records[:split], records[split:]

    # Oversample minority urgency classes in training set
    train = oversample_by_key(train, key="urgency")
    # Oversample minority department classes in training set
    train = oversample_by_key(train, key="department")
